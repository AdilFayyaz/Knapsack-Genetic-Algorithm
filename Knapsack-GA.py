...
def _calculate_weight_value(cromossome, items):
    weight = 0
    value = 0
    for x, val in enumerate(cromossome):
        if val == 1:
...
def find_two_fittest_individuals(population):
    # Sort the named tuple
    population = sorted(population, key=attrgetter('value'), reverse=True)
    highest_index = 0
    second_highest_index = 1
    #return the first two indexes from the function
...
def calculate_fitness(population, items, max_weight):
    ...
    for chromosome in population:
        weight, value = _calculate_weight_value(chromosome.cromossome, items)
        # Mutate until weight > max_weight
        while weight > max_weight:
            gene = randint(0, backpack_capacity - 1)
            chromosome.cromossome[gene] = 0
            weight, value = _calculate_weight_value(chromosome.cromossome, items)

        # Named Tuples are immutable so created a new tuple(fit_population), and appended Individual
        # named value pairs in them
        fit_population.append(
            Individual(
                cromossome=chromosome.cromossome,
                weight=weight,
                value=value
            )
        )
...
#   find weight, value = _calculate_weight_value(cromossome, items)

#       while weight > max_weight:
#          randomly make an index 0 and calculate weight again untill valid


#    Add the valid chromosome to population
